...
GPUS_PER_TASK=1
GPU_IDS=(0 1 2 3)          # ✅ 只使用这几张 GPU（物理 GPU ID）
NUM_GPUS=${#GPU_IDS[@]}
...
for clip in "${clips[@]}"; do
  for tau in "${taus[@]}"; do
    # for a in "${as[@]}"; do
      # ✅ 每个组合独立命名，避免覆盖
    # base_output_name="longvideobench_clip_${clip}_tau_${tau}_alpha_${a}trace_softmax"
    base_output_name="longvideobench_clip_${clip}_tau_${tau}_trace_softmax"
    # base_output_name='longvideobench_aks'
    ...
    # keyframe_path=None
    # keyframe_path="longvideobench_keyframes/aks_64"
    ...
    for IDX in $(seq 0 $((CHUNKS-1))); do
      ...
      python3 qwen3-vl/internvl-3.5_longvideobench.py \
        --video_dir "$video_dir" \
        --gt_file "$gt_file" \
